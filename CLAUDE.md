...
- Loss: masked softmax focal loss with sqrt-inverse-frequency class weights `[1.20, 1.00, 8.61, 8.61, 3.85, 4.25, 2.58]` (computed from intent-filtered LOKI train distribution, 367/515 scenes after removing boring STOP+MOVING+CROSSING-only scenes). `ped_loss_weight=1.0` (LOKI has proportionally more pedestrians than nuScenes).
