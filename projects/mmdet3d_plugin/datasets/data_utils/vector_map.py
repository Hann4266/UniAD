...
class VectorizedLocalMap(object):
    ...
    def sample_pts_from_line(self, line):
        # print("LINE geom_type:", line.geom_type, "bounds:", line.bounds)
        if self.fixed_num < 0:
        ...
        if self.normalize:
            sampled_points = sampled_points / np.array([self.patch_size[0], self.patch_size[1]])

        num_valid = len(sampled_points)

        if not self.padding or self.fixed_num > 0:
            # print("DEBUG minx/miny:", sampled_points[:,0].min(), sampled_points[:,1].min())
