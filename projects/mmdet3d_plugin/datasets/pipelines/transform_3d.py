...
class ObjectRangeFilterTrack(object):
    ...
    def __call__(self, input_dict):
        ...
        #BEV
        centers = gt_bboxes_3d.tensor[:, :3]  # (N, 3), torch
        x = centers[:, 0]  
        y = centers[:, 1]  

        # BEV range: [x_min, y_min, x_max, y_max] [0, -51.2, 51.2, 51.2]
        x_min, y_min, x_max, y_max = [float(v) for v in bev_range]
        ...
        # FOV
        half_fov_rad = np.deg2rad(70 / 2.0)
        tan_half_fov = float(np.tan(half_fov_rad))
        
        # related position
        forward_dist = y - ego_y  # Forward distance
        lateral_dist = (x - ego_x).abs()  # Absolute lateral distance

        mask_fov = (forward_dist > 0) & (lateral_dist <= forward_dist * tan_half_fov)
        mask_x = (x >= x_min) & (x <= x_max)
        mask_y = (y >= y_min) & (y <= y_max)
        mask = mask_fov & mask_x & mask_y
