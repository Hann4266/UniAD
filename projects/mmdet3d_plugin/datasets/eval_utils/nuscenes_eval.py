...
# from .loaders import load_prediction, load_gt, add_center_dist, filter_eval_boxes
...
def vis_bbox(nusc, gt_bboxes, pred_bboxes, out_path="lane_vis.png", dpi=200):
    for sample_token in gt_bboxes.boxes.keys():
        ...
        # Ego position in GLOBAL frame
        ego_x   = pose_record['translation'][0]
        ego_y   = pose_record['translation'][1]
        
        # Ego heading from quaternion
        ego_q   = Quaternion(pose_record['rotation'])
        yaw_rad = ego_q.yaw_pitch_roll[0]
        yaw_deg = np.degrees(yaw_rad)
        
        # Heading arrow tip in global frame
        arrow_len = 5.0  # meters
        arrow_dx  = arrow_len * np.cos(yaw_rad)   # global x component
        arrow_dy  = arrow_len * np.sin(yaw_rad)   # global y component

        # ---- Plot 2: merged lanes by type with colors ----
        ax = axes[1]
        ax.set_title("detection_prediction")
         # Draw ego vehicle at its ACTUAL global position
        ax.plot(ego_x, ego_y, 's', color='black', markersize=12,
                label='Ego', zorder=5)

        # Draw heading arrow from ego global position
        ax.annotate(
            '',
            xy=(ego_x + arrow_dx, ego_y + arrow_dy),   # arrow tip (global)
            xytext=(ego_x, ego_y),                      # arrow tail (ego global pos)
            arrowprops=dict(arrowstyle='->', color='black', lw=2.5),
            zorder=6
        )

        for bbox in pred_bboxes[sample_token]:
            if bbox.detection_name == "car":
                x, y = bbox.translation[0], bbox.translation[1]
                ax.scatter(x, y, c="b", s=100)
        for bbox in gt_bboxes[sample_token]:
            if bbox.detection_name == "car":
                x, y = bbox.translation[0], bbox.translation[1]
        ...
        # ax.set_xlim(xlim)
        # ax.set_ylim(ylim)
        save_path = os.path.join(out_path, f"{sample_token}.png")        
        os.makedirs(out_path, exist_ok=True)  # ← ADD THIS LINE
...
class NuScenesEval_custom(NuScenesEval):
    ...
    def __init__(self,
        ...
        self.gt_boxes = filter_eval_boxes(nusc, self.gt_boxes, self.cfg.class_range, filter_fov = True, verbose=verbose)
        
        # vis_bbox(nusc, self.gt_boxes, self.pred_boxes, out_path=f"/zihan-west-vol/UniAD/projects/work_dirs/3d", dpi=250)
