...
class NuScenesMapExplorer:
    ...
    def _get_layer_geom(self,
        ...
        """
        if layer_name in self.map_api.non_geometric_polygon_layers:
            return self._get_layer_polygon_new(patch_box, patch_angle, layer_name)
        elif layer_name in self.map_api.non_geometric_line_layers:
            return self._get_layer_line_new(patch_box, patch_angle, layer_name)
        else:
            raise ValueError("{} is not a valid layer".format(layer_name))
    ...
    def _polygon_geom_to_mask(self,
        ...
        patch = self.get_patch_coord_new(local_box)
        ...
        patch_mask = np.zeros(canvas_size, np.uint8)
        ...
        #     boundary_line = LineString([coords[i], coords[i + 1]])
        #     # Apply same transformations as map lines
        #     boundary_line = affinity.affine_transform(
        #         boundary_line, [1.0, 0.0, 0.0, 1.0, trans_x, trans_y])
        #     boundary_line = affinity.scale(
        #         boundary_line, xfact=scale_width, yfact=scale_height, origin=(0, 0))
            
        #     # Draw boundary line on patch_mask
            
        #     patch_mask = self.mask_for_lines(boundary_line, patch_mask)

        # # ========== Mask out overlapping pixels ==========
        # # Where both masks have value 1, set map_mask to 0
        # overlap = (map_mask == 1) & (patch_mask == 1)
        # map_mask[overlap] = 0
        return map_mask
    ...
    def _get_layer_polygon_new(self,
                           patch_box: Tuple[float, float, float, float],
                           patch_angle: float,
        ...
        patch = self.get_patch_coord_new(patch_box, patch_angle)

        records = getattr(self.map_api, layer_name)
        # clip_region = box(0, -patch_h/2.0, patch_w/2.0, patch_h/2.0)
        polygon_list = []
        if layer_name == 'drivable_area':
            for record in records:
                polygons = [self.map_api.extract_polygon(polygon_token) for polygon_token in record['polygon_tokens']]
                ...
                for polygon in polygons:
                    ...
                    if not new_polygon.is_empty:
                        ...
                        # new_polygon = new_polygon.intersection(clip_region)
                        # if not new_polygon.is_empty:
                        #     if new_polygon.geom_type == 'Polygon':
                        #         new_polygon = MultiPolygon([new_polygon])
    ...
    def _get_layer_line_new(self,
        ...
        for record in records:
            ...
            if not new_line.is_empty:
                ...
                new_line = affinity.affine_transform(new_line,
                                                     [1.0, 0.0, 0.0, 1.0, -patch_x, -patch_y])
                # right_half_box = box(0, -patch_h/2.0, patch_w/2.0, patch_h/2.0)
                # new_line = new_line.intersection(right_half_box)
                # if not new_line.is_empty:
                    # print("geom:", new_line.geom_type, "bounds:", new_line.bounds)  # minx,miny,maxx,maxy
    ...
    #                 fov_angle: float = 70.0) -> Polygon:
    #     """
    #     Create a triangular FOV patch with apex at ego vehicle.
    #     :param patch_box: [x_center, y_min, height, width] where height is y_max range.
    #     :param patch_angle: Vehicle heading in degrees.
    #     :param fov_angle: Total field of view angle in degrees (e.g., 70° = ±35°).
    #     :return: Triangular Polygon representing the FOV.
    #     """
    ...
    def get_patch_coord(patch_box: Tuple[float, float, float, float],
        ...
        return patch
    @staticmethod
    def get_patch_coord_new(patch_box: Tuple[float, float, float, float],
                        patch_angle: float = 0.0) -> Polygon:
        """
        Convert patch_box to shapely Polygon coordinates.
        :param patch_box: Patch box defined as [x_center, y_center, height, width].
