"""
Fine-tune nuScenes-pretrained UniAD on LOKI dataset.

Loads an adapted nuScenes checkpoint (10 classes -> 8 classes, seg_head removed)
and fine-tunes BEVFormer encoder + detection/tracking decoder on LOKI data.
Map head is NOT included (LOKI has no map GT).

Prerequisites:
    1. Run the weight adaptation script:
    ...
    2. Train:
       cd /mnt/storage/UniAD && PYTHONPATH=$(pwd):$PYTHONPATH python -u tools/train.py \
           ...
           --deterministic --cfg-options log_config.interval=1
"""
...
# --------------------------------------------------------------------- #
#  Point cloud / BEV range (same as base_loki_perception.py)
# --------------------------------------------------------------------- #
...
# --------------------------------------------------------------------- #
#  LOKI 8 classes (same as base_loki_perception.py)
# --------------------------------------------------------------------- #
class_names = [
    "Pedestrian",
    "Car",
...
# --------------------------------------------------------------------- #
#  Architecture dimensions (must match nuScenes checkpoint)
# --------------------------------------------------------------------- #
_dim_ = 256
_pos_dim_ = _dim_ // 2
_ffn_dim_ = _dim_ * 2
...
# --------------------------------------------------------------------- #
#  Model config — perception only (BEVFormer + tracking), NO map head
# --------------------------------------------------------------------- #
...
model = dict(
    ...
    # Freeze backbone (pretrained ImageNet, same as from-scratch LOKI training)
    freeze_img_backbone=True,
    # Neck + BEV encoder + decoder all trainable (fine-tuned from nuScenes)
    freeze_img_neck=False,
    freeze_bn=False,
    # Do NOT freeze BEV encoder — we want to fine-tune it on LOKI
    # (same as base_loki_perception.py from-scratch training)
    score_thresh=0.4,
    ...
    # No seg_head (map module) — LOKI has no map ground truth
    # No motion_head, occ_head, planning_head
    train_cfg=dict(
        pts=dict(
            grid_size=[512, 512, 1],
            voxel_size=voxel_size,
...
# --------------------------------------------------------------------- #
#  Dataset config (identical to base_loki_perception.py)
# --------------------------------------------------------------------- #
...
train_pipeline = [
    ...
    # GT filters: range, name, FOV (60°), camera visibility (2D bbox check)
...
# --------------------------------------------------------------------- #
#  Optimizer / schedule — lower LR for fine-tuning
# --------------------------------------------------------------------- #
optimizer = dict(
    type="AdamW",
    lr=5e-5,  # lower than from-scratch (2e-4) for fine-tuning stability
    paramwise_cfg=dict(
...
# Load adapted nuScenes checkpoint (run adapt_nuscenes_to_loki.py first)
load_from = "/mnt/storage/UniAD/work_dirs/zihan_nuscenes_weight/nuscenes_adapted_for_loki.pth"
