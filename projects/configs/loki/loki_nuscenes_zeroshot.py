"""
Zero-shot inference config: nuScenes-trained model on LOKI data.

Uses the nuScenes model architecture (10 classes, seg_head for map) but
feeds LOKI single-camera data padded to 6 cameras. This tests whether the
nuScenes map head produces useful features on LOKI scenes.

Checkpoint: work_dirs/zihan_nuscenes_weight/epoch_6.pth
"""
...
# Must match the nuScenes checkpoint's training config
...
# nuScenes 10 classes (matching checkpoint)
class_names = [
...
# ------------------------------------------------------------------- #
# Model: nuScenes architecture with seg_head (map), no intent/motion/occ/planning
# ------------------------------------------------------------------- #
model = dict(
    type="UniAD",
    ...
    # Map segmentation head (PansegformerHead) — matches nuScenes checkpoint
    seg_head=dict(
        ...
        stuff_transformer_head=dict(
            type='SegMaskHead', d_model=_dim_, nhead=8, num_decoder_layers=6,
            self_attn=True),
        train_cfg=dict(
            assigner=dict(
    ...
    # No intent/motion/occ/planning (not in checkpoint)
    train_cfg=dict(
        pts=dict(
            grid_size=[512, 512, 1],
            voxel_size=voxel_size,
...
# ------------------------------------------------------------------- #
# Dataset: LOKI with camera padding for nuScenes model
# ------------------------------------------------------------------- #
...
test_pipeline = [
    dict(type="LoadLokiImage", to_float32=True, target_size=(1600, 900)),
    # Pad single camera to 6 cameras for nuScenes model compatibility
    dict(type="PadToMultiCamera", target_num_cams=6),
