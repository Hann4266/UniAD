"""
LOKI dataset configuration for UniAD perception pipeline.

Key differences from nuScenes config:
    - Single camera (num_cams=1) vs 6 surround cameras
    - 6 classes (car, truck, bus, pedestrian, motorcycle, bicycle)
    - queue_length=3 for memory efficiency
    - Map/Occ/Motion/Planning heads disabled (perception only)
    - LOKI-specific data loading pipeline
"""
...
# --------------------------------------------------------------------- #
# LOKI objects can be far (up to ~120m), but we keep a reasonable range
# for BEV. Rotated 90° CCW to match nuScenes convention: x=right, y=forward.
point_cloud_range = [-51.2, 0, -5.0, 51.2, 51.2, 3.0]
voxel_size = [0.2, 0.2, 8]
patch_size = [51.2, 102.4]

# ImageNet mean/std for BGR (caffe-style, same as nuScenes config)
img_norm_cfg = dict(
...
# --------------------------------------------------------------------- #
#  LOKI classes (2 classes: car + pedestrian)
# --------------------------------------------------------------------- #
class_names = [
    "Pedestrian", 
    "Car", 
...
# Vehicle class indices (for occupancy, not used but kept for compat)
vehicle_id_list = [0]
group_id_list = [[0], [1]]
...
fut_steps = 4
past_steps = 4
use_nonlinear_optimizer = False  # no nuScenes predict helper

# Occ settings (dummy, not used)
occ_n_future = 4
occ_n_future_plan = 6
...
# --------------------------------------------------------------------- #
#  Model config - perception only (tracking + detection)
# --------------------------------------------------------------------- #
...
model = dict(
    ...
    num_query=900,  # must match pretrained BEVFormer weights
    ...
    loss_cfg=dict(
        ...
        loss_past_traj_weight=0.0,  # no trajectory loss in perception
    ),
    pts_bbox_head=dict(
    ...
    # Map segmentation head disabled for perception-only training
    # seg_head=None,
...
train_pipeline = [
    ...
    dict(type="LoadAnnotations3D_E2E",
         with_bbox_3d=True,
         with_label_3d=True,
         with_future_anns=False,  # No occ for LOKI
         with_ins_inds_3d=True,
         ins_inds_add_1=True),
    # LOKI-specific: dummy occ labels for pipeline compat
    dict(type="GenerateDummyOccLabels",
         bev_h=bev_h_, bev_w=bev_w_,
         n_future=occ_n_future_max),
    # Standard NuScenes filters
...
data = dict(
    samples_per_gpu=1,
    workers_per_gpu=4,  # reduced for memory
    train=dict(
        type=dataset_type,
        data_root=data_root,
...
# Load pretrained BEVFormer backbone weights
